...
int Gatekeeper_Initialize(void)
{
  ...
  success = TEEC_OpenSession(gGatekeeperContext, gGatekeeperSession, (const TEEC_UUID *)&GatekeeperUUID[0],
    ...
    uint8_t **enrolled_password_handle, uint32_t *enrolled_password_handle_length)
{
  ...
  TEEC_Result success = TEEC_InvokeCommand(gGatekeeperSession, MSG_GATEKEEPER_ENROLL_PASS_UID, &operation, NULL);
  //ALOGE("success=0x%x\n", success);
  if(success) {
  ...
  //ALOGE("enrolled password size = %d\n", operation.params[3].memref.size);
  if(!success) {
    ...
    bool *request_reenroll)
{
  ...
  TEEC_Result success = TEEC_InvokeCommand(gGatekeeperSession, MSG_GATEKEEPER_VERIFY_PASS_UID, &operation, NULL);
  if(success) {
