...
//UUID for keymaster, it is a demo.
...
int KeyMaster_Generate_keypairs(const keymaster_device_t* dev,
        ...
        uint8_t** keyBlob, size_t* keyBlobLength)
{
    //ALOGE("KeyMaster_Generate_keypairs XXXXXXXX\n");
...
{
    int ret = 0;
    //ALOGE("KeyMaster_Import_keyparis XXXXXXXX\n");
    if (dev->context == NULL) {
        ...
        size_t* x509_data_length)
{
    ...
    //ALOGE("KeyMaster_Get_keyparis_public XXXXXXXX\n");
    if (dev->context == NULL) {
    ...
    if (!success) {
        ...
        key_length = operation.params[1].memref.size;
...
static int sunxi_tee_keymaster_device_open(const hw_module_t* module, const char* name,
    ...
    const char *test_buf = {"All WinnerTech keymaster module: base on tee: init!"};
    int ret = 0;

    if (strcmp(name, KEYSTORE_KEYMASTER) != 0)
        return -EINVAL;

    ALOGE("%s:: Enter AW tee keymaster\n",__func__);
