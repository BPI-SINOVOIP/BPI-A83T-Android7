...
static void select_input_devices(struct sunxi_audio_device *adev)
{
    ALOGV("select_input_devices");

    char in_path[50] = "null";
    char pdev_name[50] ="";
    int in_pdev = 0;

    in_pdev = get_platform_device(adev->mode, adev->in_devices, adev->platform);

    ALOGD("select_input_devices:mode(%#x), in_devices(%#x), cur_in_device(%d), in_pdev(%d).",
                    adev->mode, adev->in_devices, adev->cur_in_device, in_pdev);

    /* select input device */
    if (adev->cur_in_device != in_pdev) {
        ...
        adev->cur_in_device = in_pdev;
        ALOGD("select device(in):pdev:%s, path:%s", pdev2str(pdev_name, in_pdev), in_path);
        ...
        ALOGV("in device not change, do nothing.");
    }
}

static void select_output_devices(struct sunxi_audio_device *adev)
{
    ALOGV("select_output_devices");
    ...
    ALOGD("select_output_devices:mode(%#x), out_devices(%#x), cur_out_device(%d), out_pdev(%d).",
                adev->mode, adev->out_devices, adev->cur_out_device, out_pdev);

    /* select output device */
    if (adev->cur_out_device != out_pdev) {
        ...
        adev->cur_out_device = out_pdev;
        ALOGD("select device(out):pdev:%s, path:%s", pdev2str(pdev_name, out_pdev), out_path);
	 update_platform_path(adev->platform);
        ...
        ALOGV("out device not change, do nothing.");
    }
}

static void select_phone_devices(struct sunxi_audio_device *adev)
{
    ALOGV("select_phone_devices");
    ...
    ALOGD("select_phone_devices:mode(%#x), phone_pdev(%d).", adev->mode, phone_pdev);

    /* select phone device */
    //reset_platform_path(adev->platform);
    get_platform_path(phone_path, adev->platform, phone_pdev);
    apply_platform_path(adev->platform, phone_path);
    ALOGD("select device(phone):pdev:%s, path:%s", pdev2str(pdev_name, phone_pdev), phone_path);
    update_platform_path(adev->platform);
...
static void select_devices(struct sunxi_audio_device *adev, int direction)
{
    if(adev->mode == AUDIO_MODE_IN_CALL){
        select_phone_devices(adev);
    }
...
#ifndef OCTOPUS_AUDIO_DEVICE
    /* load mixer config from mixer xml file */
    if(out->card == 0){
        if(adev->platform->ar == NULL){
            adev->platform->ar = audio_route_init(out->card, MIXER_PATHS_XML_PATH);
        }
    }
#endif
    select_devices(adev, OUTPUT);
...
static int adev_set_parameters(struct audio_hw_device *dev, const char *kvpairs)
{
    ...
    if (ret >= 0) {
        if (strcmp(value, AUDIO_PARAMETER_VALUE_OFF) == 0) {
            #ifndef OCTOPUS_AUDIO_DEVICE
            audio_route_free(adev->platform->ar);
            adev->platform->ar = NULL;
            #endif
        }
    }
                     ...
                     hw_device_t** device)
{
    ...
    *device = &adev->device.common;
    ...
    adev->cur_in_device = 0;
    adev->cur_out_device = 0;
