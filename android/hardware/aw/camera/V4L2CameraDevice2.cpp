...
//modify for cts by clx
    ...
    if (mTakePictureState == TAKE_PICTURE_NORMAL)
    {
        ...
        mPicBuffer.addrVirY = (unsigned long)mMemOpsS->palloc_cam(frame_size, SHARE_BUFFER_FD,&mPicBuffer.nShareBufFd);
...
#ifdef USE_ION_MEM_ALLOCATOR
            mVideoBuffer.buf_vir_addr[i] = (unsigned long)mMemOpsS->palloc_cam(buffer_len,SHARE_BUFFER_FD,&mVideoBuffer.nDmaBufFd[i]);
