...
// Sum of CPU_NUM + 2 for GPU, BATTERY.
...
const int CPU_SENSORS[] = {0};
const char *CPU_LABEL[]={"CPU0","CPU1","CPU2","CPU3"};

#define CPU_NUM                 (sizeof(CPU_SENSORS) / sizeof(int))
//therm-reset-temp
#define CPU_SHUTDOWN_THRESHOLD   110
//limit-temp
#define CPU_THROTTLING_THRESHOLD 65


#define GPU_THROTTLING_THRESHOLD 95
#define GPU_SHUTDOWN_THRESHOLD   110
#define GPU_SENSOR_NUM           1
...
static ssize_t read_temperature(int sensor_num, int type, const char *name, float mult,
    ...
    if (file == NULL) {
        ALOGE("%s: failed to open: %s ,file_name=%s\n", __func__, strerror(errno),file_name);
        return -errno;
    }

    int ret= fscanf(file, "%f", &temp);
    if (1 != ret) {
        fclose(file);
        ALOGE("%s: failed to read a float: %s ,file_name =%s, temp=%f\n", __func__, strerror(errno),file_name,temp);
...
static ssize_t get_temperatures(thermal_module_t *module, temperature_t *list, size_t size) {
    ...
    if (current_index < size) {
        result = read_temperature(GPU_SENSOR_NUM, DEVICE_TEMPERATURE_GPU, GPU_LABEL, 1,
                GPU_THROTTLING_THRESHOLD, GPU_SHUTDOWN_THRESHOLD, UNKNOWN_TEMPERATURE,
                &list[current_index]);
        if (result < 0) {
            return result;
        }
