...
static int hwc_blank(struct hwc_composer_device_1* dev, int disp, int blank)
{
    ...
    if (blank) {
        arg[0] = 0;
        arg[1] = 0;
        if (ioctl(ctx->disp_fp, DISP_CMD_VSYNC_EVENT_EN,(unsigned long)arg) != 0) {
            ALOGE("hwc blank failed!");
            return -1;
        }
        ctx->vsync_en = 0;
    } else {
    ...
    arg[0] = 0;
    arg[1] = blank;
    ALOGV("hwc blank %d!", blank);
    if(ioctl(ctx->disp_fp, DISP_CMD_BLANK, (unsigned long)arg) != 0) {
        ALOGE("hwc blank failed!");
        return -1;
    }
    return 0;
