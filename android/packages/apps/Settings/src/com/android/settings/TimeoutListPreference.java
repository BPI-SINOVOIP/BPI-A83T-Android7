...
public class TimeoutListPreference extends RestrictedListPreference {
    ...
    public void removeUnusableTimeouts(long maxTimeout, EnforcedAdmin admin) {
        boolean isShowNeverTimeout = getContext().getResources().getBoolean(
        ...
        if (isShowNeverTimeout) {
            ...
            for (int i = 0; i < mInitialValues.length; i++) {
                newEntries.add(mInitialEntries[i]);
                newValues.add(mInitialValues[i]);
            }
            newEntries.add(getContext().getString(R.string.screen_off_never_sleep_summary));
            newValues.add(String.valueOf(Integer.MAX_VALUE));
            setEntries(newEntries.toArray(new CharSequence[0]));
            setEntryValues(newValues.toArray(new CharSequence[0]));
        }
