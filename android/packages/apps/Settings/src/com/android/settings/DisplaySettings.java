...
public class DisplaySettings extends SettingsPreferenceFragment implements
    ...
    private static final String KEY_COLOR_ENHANCE = "color_enhance";
    private static final String KEY_COLOR_ENHANCE_DEMO = "color_enhance_demo";
    ...
    private static final String KEY_READING_MODE_STRENGTH_SCALE = "reading_mode_strength_scale";
    ...
    private SwitchPreference mHdmiFullScreenPreference;
    ...
    private SwitchPreference mColorEnhancePreference;
    private SwitchPreference mColorEnhanceDemoPreference;
    private SwitchPreference mSmartBacklightPreference;
    private SwitchPreference mSmartBacklightDemoPreference;
    private SwitchPreference mReadingModePreference;
    private SeekBarPreference mReadingModeStrengthScale;
    private SeekBarPreference mColorTempteratureScale;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mHdmiFullScreenPreference = (SwitchPreference) findPreference(KEY_HDMI_FULL_SCREEN);
        mHdmiOutputModePreference = (ListPreference) findPreference(KEY_HDMI_OUTPUT_MODE);
        mHdmiWidthScale = (SeekBarPreference) findPreference(KEY_HDMI_WIDTH_SCALE);
        mHdmiHeightScale = (SeekBarPreference) findPreference(KEY_HDMI_HEIGHT_SCALE);
        if (mHdmiFullScreenPreference != null) {
            mHdmiFullScreenPreference.setOnPreferenceChangeListener(this);
            // final boolean isHdmiFullScreen = Settings.System.getInt(resolver, Settings.System.HDMI_FULL_SCREEN, 0) > 0;
            mHdmiFullScreenPreference.setChecked(false);
        ...
        mColorEnhancePreference = (SwitchPreference) findPreference(KEY_COLOR_ENHANCE);
        mColorEnhanceDemoPreference = (SwitchPreference) findPreference(KEY_COLOR_ENHANCE_DEMO);
        if (mColorEnhancePreference != null) {
            boolean enable = AWDisplay.getColorEnhance();
            mColorEnhancePreference.setChecked(enable);
            mColorEnhancePreference.setOnPreferenceChangeListener(this);
            mColorEnhanceDemoPreference.setEnabled(enable);
            mColorEnhanceDemoPreference.setChecked(AWDisplay.getColorEnhanceDemo());
            mColorEnhanceDemoPreference.setOnPreferenceChangeListener(this);
        }

        int screenWidth = getActivity().getWindowManager().getDefaultDisplay().getWidth();
        if (screenWidth > 2048) {
            Log.d(TAG,"ScreenWidth exceeds the number of line buffers.Cannot support smbl");
            removePreference("smart_backlight_setting");
        }
        mSmartBacklightPreference = (SwitchPreference) findPreference(KEY_SMART_BACKLIGHT);
        mSmartBacklightDemoPreference = (SwitchPreference) findPreference(KEY_SMART_BACKLIGHT_DEMO);
        if (mSmartBacklightPreference != null) {
            boolean enable = AWDisplay.getSmartBacklight();
            mSmartBacklightPreference.setChecked(enable);
            mSmartBacklightPreference.setOnPreferenceChangeListener(this);
            mSmartBacklightDemoPreference.setEnabled(enable);
            mSmartBacklightDemoPreference.setChecked(AWDisplay.getSmartBacklightDemo());
            mSmartBacklightDemoPreference.setOnPreferenceChangeListener(this);
        }

        mReadingModePreference = (SwitchPreference) findPreference(KEY_READING_MODE);
        mReadingModeStrengthScale= (SeekBarPreference) findPreference(KEY_READING_MODE_STRENGTH_SCALE);
        mColorTempteratureScale= (SeekBarPreference) findPreference(KEY_COLOR_TEMPERATURE_SCALE);
        if (mReadingModePreference != null && mReadingModeStrengthScale != null) {
            boolean enable = AWDisplay.getReadingModeIsEnabled();
            mReadingModePreference.setChecked(enable);
            mReadingModePreference.setOnPreferenceChangeListener(this);
            mReadingModeStrengthScale.setEnabled(enable);
            mReadingModeStrengthScale.setProgress(AWDisplay.getReadingModeStrength());
            mReadingModeStrengthScale.setOnPreferenceChangeListener(this);
            if (mColorTempteratureScale != null) {
                mColorTempteratureScale.setEnabled(!enable);
                mColorTempteratureScale.setProgress(AWDisplay.getColorTemperature());
                mColorTempteratureScale.setOnPreferenceChangeListener(this);
            }
        }
}
    ...
    public boolean onPreferenceChange(Preference preference, Object objValue) {
        ...
        if (preference == mHdmiFullScreenPreference) {
            boolean value = mHdmiFullScreenPreference.isChecked();
        }
        if (preference == mHdmiOutputModePreference) {
        }
        if (preference == mHdmiWidthScale) {
        }
        if (preference == mHdmiHeightScale) {
        }
        if (preference == mColorEnhancePreference) {
            boolean value = (Boolean) objValue;
            AWDisplay.setColorEnhance(value);
            mColorEnhanceDemoPreference.setChecked(AWDisplay.getColorEnhanceDemo());
            mColorEnhanceDemoPreference.setEnabled(value);
        } else if (preference == mColorEnhanceDemoPreference) {
            boolean value = (Boolean) objValue;
            AWDisplay.setColorEnhanceDemo(value);
        } else if (preference == mSmartBacklightPreference) {
            boolean value = (Boolean) objValue;
            AWDisplay.setSmartBacklight(value);
            mSmartBacklightDemoPreference.setChecked(AWDisplay.getSmartBacklightDemo());
            mSmartBacklightDemoPreference.setEnabled(value);
        } else if (preference == mSmartBacklightDemoPreference) {
            boolean value = (Boolean) objValue;
            AWDisplay.setSmartBacklightDemo(value);
        }
        if (preference == mReadingModePreference) {
            boolean value = (Boolean) objValue;
            AWDisplay.setReadingMode(value, mReadingModeStrengthScale.getProgress());
            mReadingModeStrengthScale.setEnabled(value);
            mColorTempteratureScale.setEnabled(!value);
            if (!value) {
                mColorTempteratureScale.setProgress(AWDisplay.getColorTemperature());
                AWDisplay.setColorTemperature(AWDisplay.getColorTemperature());
            }
        }
        if (preference == mReadingModeStrengthScale) {
            int value = (int) objValue;
            AWDisplay.setReadingMode(true, value);
        }
        if (preference == mColorTempteratureScale) {
            int value = (int) objValue;
