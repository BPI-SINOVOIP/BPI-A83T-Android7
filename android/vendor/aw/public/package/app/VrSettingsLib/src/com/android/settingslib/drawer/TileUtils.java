...
public class TileUtils {
    ...
    public static List<DashboardCategory> getCategories(Context context,
        ...
        for (UserHandle user : userManager.getUserProfiles()) {
            ...
            if (setup) {
                Log.w(LOG_TAG, "getTilesForAction: " + EXTRA_SETTINGS_ACTION);
    ...
    public static void getTilesForIntent(Context context, UserHandle user, Intent intent,
        ...
        for (ResolveInfo resolved : results) {
            if (!resolved.system) {
                Log.d(LOG_TAG, resolved.activityInfo.name + " is not a system app");
                // Do not allow any app to add to settings, only system ones.
                continue;
            }
            ...
            if (tile == null) {
                ...
                updateTileData(context, tile, activityInfo, activityInfo.applicationInfo,
                        pm);
                if (DEBUG) Log.d(LOG_TAG, "Adding tile " + tile.title + ", intent class name: " + activityInfo.name);
