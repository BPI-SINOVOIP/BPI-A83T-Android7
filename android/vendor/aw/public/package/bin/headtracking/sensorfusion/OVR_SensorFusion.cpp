...
#define LOG_TAG "SensorFusion"

void SensorFusion::RecenterYaw()
{
    ALOGD("SensorFusion::RecenterYaw()%s", "");
    LogText("SensorFusion::RecenterYaw()%s", "");
...
void SensorFusion::RecenterOrientation()
{
    LogText("SensorFusion::RecenterOrientation()%s", "");
    // get the current state
    const StateForPrediction state = UpdatedState.GetState();
...
void SensorFusion::handleMessage(const SFMessageBodyFrame& msg)
{
    ...
    if( AlwaysImuEvent )
    {
        LogText("IMU Sample: {%f,%f}, {%f,%f,%f}, {%f,%f,%f}",
            msg.AbsoluteTimeSeconds, msg.TimeDelta,
            msg.Acceleration.x, msg.Acceleration.y, msg.Acceleration.z,
            msg.RotationRate.x, msg.RotationRate.y, msg.RotationRate.z);
    }
...
#ifdef YAW_LOGGING
    LogText("YAW magUncalib=%.3f %.3f %.3f \n",    magUncalibrated.x, magUncalibrated.y, magUncalibrated.z);
#endif
    Vector3f magCalibrated = magUncalibrated - magBias;
    Vector3f magInWorldFrame = orientation.Rotate(magCalibrated);// {magCalibrated, t = T10},  {orientation, t = Tx}
...
#ifdef YAW_LOGGING
        LogText("YAW, mag correction occur");
...
Posef SensorFusion::calcPredictedPose(const PoseStatef& poseState, float predictionDt ) const
{
    ...
    // For first frame when after wakeup:
    // if bNeedSyncTime(set by Resume) have not taken effect,
    // after wakeup, delta may be decreased. Only when delta is negative, it can be detected.
    // if delta >= 0 and delta have been decreased, it can not be detected.

    if(predictionDt < 0 ) {
        // not error, now we just will meet this issue in 1~2 seconds.
        // ALOGD("predictionDt is %f < 0, reset sensor hal timeline", predictionDt);
        if (mSensorDevice != NULL) {
            mSensorDevice->setNeedSyncTime(true);
        }
        return poseState.Transform;
    }
    ...
    if (angularSpeed > 0.001) {
        ...
        if( AlwaysImuEvent ) {
            LogText("dynamicDt = %f, predictionDt = %f, candidateDt = %f, angularSpeed = %f, linearSpeed = %f, speed = %f, AngularVelocity = (%f, %f, %f), LinearVelocity = (%f, %f, %f)",
            dynamicDt, predictionDt, candidateDt,
            angularSpeed, poseState.LinearVelocity.Length(), speed,
            angularVelocity.x, angularVelocity.y, angularVelocity.z,
            poseState.LinearVelocity.x, poseState.LinearVelocity.y, poseState.LinearVelocity.z);
        }
    }
...
void SensorFusion::OnResume() {
    ALOGD("SensorFusion::OnResume");
    //RecenterYaw();
    if (mSensorDevice != NULL) {
        mSensorDevice->setNeedSyncTime(true);
    }
...
//
void SensorFusion::setSensor(SensorHal* sensorDevice) {
