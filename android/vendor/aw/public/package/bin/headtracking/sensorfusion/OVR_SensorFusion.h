...
public:
virtual ~SensorHal(){}
virtual void setNeedSyncTime(bool needSyncTime) = 0;
};
...
class SFMessageBodyFrame
{
public:
    // JDC: added this so I can have an array of them
    SFMessageBodyFrame()
        : Temperature(0.0f), TimeDelta(0.0f)
    {
    }
...
public:
    ...
    Posef calcPredictedPose(const PoseStatef& poseState, float predictionDt ) const;
    ...
    void        OnMessage(const SFMessageBodyFrame& msg)
    {
    ...
    void        handleMessage(const SFMessageBodyFrame& msg);

    void        SetAlwaysLogImuEvent(bool enable)
    {
            AlwaysImuEvent = enable;
    }

    void OnResume();

    void setSensor(SensorHal* sensorDevice);
...
private:
    ...
    bool                    AlwaysImuEvent; // used to enable or diable imu event log.

    SensorHal* mSensorDevice = NULL;
};
