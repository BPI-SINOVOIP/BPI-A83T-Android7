...
struct ArSpiBoardCfg {
    uint8_t gpioMiso;
    uint8_t gpioMosi;
    uint8_t gpioSclk;
    uint8_t gpioNss;

    enum ArGpioAltFunc gpioFunc;
    enum ArGpioSpeed gpioSpeed;
    enum GpioPullMode gpioPull;

    IRQn_Type irqNss;

    struct ArSpiDmaCfg dmaRx;
    struct ArSpiDmaCfg dmaTx;
...
extern const struct ArSpiBoardCfg *boardArSpiCfg(uint8_t busId);
