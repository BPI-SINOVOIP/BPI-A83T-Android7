...
static inline const struct NanohubCommand *hostIntfFindHandler(uint8_t *buf, size_t size, uint32_t *seq)
{
	...
	//osLog(LOG_ERROR, "%s-%u,%u, %u, %u\n", __func__, __LINE__, size, packet->len, NANOHUB_PACKET_SIZE(packet->len));
        return NULL;
    }
    ...
    //osLog(LOG_ERROR, "%s-%u:%lu,%lu\n", __func__, __LINE__, mTxRetrans.seq, packet->seq);
        ...
        HostIntfCommCallbackF callback)
{
    ...
    //osLog(LOG_ERROR, "%s-%u:len:%p,%u,%p\n", __func__, __LINE__, txPacket, txPacket->len, &mTxBuf.prePreamble);
    //hostIntfTxBuf(1+NANOHUB_PACKET_SIZE(len), &mTxBuf.prePreamble, callback);
    hostIntfTxBuf(NANOHUB_PACKET_SIZE(len), mTxBuf.buf, callback);
}
...
{
    if (!err && tx < mTxSize) {
    	    //osLog(LOG_ERROR, "%s-%u:tx:%u, mTxSize:%u, err:%d\n", __func__, __LINE__, tx, mTxSize, err);
        mTxSize -= tx;
        mTxBufPtr += tx;
...
{
    //osLog(LOG_ERROR, "hostIntfRxPacket:%u\n", wakeupActive);
    if (mWakeActive) {
...
static void hostIntfRxDone(size_t rx, int err)
{
    ...
    //osLog(LOG_ERROR, "rxd:%lld, %u\n", mRxTimestamp, mRxSize);
    if (err != 0) {
...
static void hostIntfGenerateAck(void *cookie)
{
    ...
    //osLog(LOG_ERROR, "%s-%u:%p\n", __func__, __LINE__, mRxCmd);
    ...
    if (mRxCmd) {
        ...
        //osLog(LOG_ERROR, "%s-%u,%d\n", __func__, __LINE__, mBusy);
        if (mBusy)
...
static void hostIntfTxAckDone(size_t tx, int err)
{
    //osLog(LOG_ERROR, "%s-%u:size:%u, err:%d\n", __func__, __LINE__, tx, err);
    //osLog(LOG_ERROR, "%s-%u:size:46, err:0\n", __func__, __LINE__);
    //udelay(60000);
...
static void hostIntfHandleEvent(uint32_t evtType, const void* evtData)
{
	...
	//osLog(LOG_INFO, "sensor config->sensType:%u\n", cmd->sensType);
	...
	sensor = mActiveSensorTable + mSensorList[(evtType & 0xFF) - 1];
        ...
        if (sensor->sensorHandle) {
		    ...
		    #if 0
		    osLog(LOG_ERROR, "%s-%u,%u,%lu,%llu,%lu,%u,%u\n",
		    	__func__,
		    	__LINE__,
		    	sensor->buffer.length,
		    	sensor->buffer.single[sensor->buffer.firstSample.numSamples].deltaTime,
		    	sensor->lastTime,
		    	sensor->buffer.single[sensor->buffer.firstSample.numSamples].idata,
		    	sensor->buffer.firstSample.numSamples,
		    	sensor->curSamples
		    	);
		    #endif
            ...
            sensorTime = sensorGetTime();
	    #if 0
	    osLog(LOG_ERROR, "%s-%u:%llu, %llu, %llu, %llu\n",
	    	__func__, __LINE__,
	    	sensorTime,
	    	sensor->firstTime,
	    	sensor->latency,
	    	sensorGetCurLatency(sensor->sensorHandle));
            #endif
            if (sensor->firstTime &&
