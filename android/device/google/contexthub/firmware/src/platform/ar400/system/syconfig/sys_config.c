...
#if SYSCONFIG_USED
//DECLARE_GLOBAL_DATA_PTR; /* uboot used, cpus use 0x44000000 */
...
unsigned int script_parser_subkey( script_main_key_t* main_name,char *subkey_name , unsigned int *pattern)
{
...
unsigned int script_parser_fetch_subkey_start(char *main_name)
{
	char   main_bkname[32];
	char   *main_char;
	script_main_key_t  *main_key = NULL;
	...
	for (i=0;i<gd->script_main_key_count;i++) {
		main_key = (script_main_key_t *)(gd->script_mod_buf + (sizeof(script_head_t)) + \
		                                 i * sizeof(script_main_key_t));
		if (strcmp(main_key->main_name, main_char)) {
			continue;
		}

		return (unsigned int)main_key;
	}

	return 0;
}

int script_parser_fetch_subkey_next(unsigned int hd, char *sub_name, int value[], int *index)
{
...
int script_parser_patch_all(char *main_name, void *str, unsigned int data_count)
{
	script_main_key_t  *main_key = NULL;
	script_sub_key_t   *sub_key = NULL;
	int    i, j;
...
************************************************************************************************************
*/
u32 gpio_request_ex(char *main_name, const char *sub_name)
{
...
__s32  gpio_get_all_pin_status(u32 p_handler, user_gpio_set_t *gpio_status, __u32 gpio_count_max, __u32 if_get_from_hardware)
{
    ...
    else
    {
        ...
        for(i = first_port; i < group_count_max; i++)
        {
            ...
            pre_port_num_func = port_num_func;
            pre_port          = port;
            script_gpio->pull      = (*tmp_group_pull_addr   >> ((port_num - (port_num_pull<<4))<<1)) & 0x03;
            ...
            {
                script_gpio->data  = (*tmp_group_data_addr   >>   port_num) & 0x01;
            }
            else
            {
                script_gpio->data = -1;
...
__s32  gpio_get_one_pin_status(u32 p_handler, user_gpio_set_t *gpio_status, const char *gpio_name, __u32 if_get_from_hardware)
{
    ...
    for(i = 0; i < group_count_max; i++)
    {
        ...
        else
        {
			...
			{
				gpio_status->mul_sel   = (R_PIO_REG_CFG_VALUE(port, port_num_func)>>tmp_val1) & 0x07;
				gpio_status->pull      = (R_PIO_REG_PULL_VALUE(port, port_num_pull)>>tmp_val2) & 0x03;
				gpio_status->drv_level = (R_PIO_REG_DLEVEL_VALUE(port, port_num_pull)>>tmp_val2) & 0x03;
			}
