...
HANDLE systimer = NULL;

//the table of timers
static struct timer timers[TIMERC_TIMERS_NUMBER] =
{
	{0, TIMER_FREE, R_TIMER0_IRQn, 0, 0, NULL, NULL},
	{1, TIMER_FREE, R_TIMER1_IRQn, 0, 0, NULL, NULL},
};
...
#ifdef  FPGA_PLATFORM
		timers[index].us_ticks        =  0;        //fix to 32k
		timers[index].ms_ticks        =  32;        //fix to 32k
#else
		timers[index].us_ticks        =  24; //24M
		timers[index].ms_ticks        =  24000; //24M
#endif
	}
...
*********************************************************************************************************
*/
int timer_start(HANDLE htimer, u32 period, u32 mode)
{
	struct timer *ptimer = (struct timer *)htimer;
	...
	//set timer period
	ptimer->pregs->interval_value = ptimer->us_ticks * period;
...
int timer_hosc_onoff_cb(u32 message, u32 aux)
{
#ifdef  FPGA_PLATFORM
	return OK;
#else
	...
	switch(message)
	{
		...
		case CCU_HOSC_ON_READY_NOTIFY:
		{
			...
			for (index = 0; index < TIMERC_TIMERS_NUMBER; index++)
			{
				...
				timers[index].pregs->control &= ~(0x3 << 2);
				timers[index].pregs->control |=  (0x1 << 2); //source clock = 24M
				timers[index].us_ticks        = 24;   //24k
				timers[index].ms_ticks        = 24000;   //24k
			}
			return OK;
		}
		default:
