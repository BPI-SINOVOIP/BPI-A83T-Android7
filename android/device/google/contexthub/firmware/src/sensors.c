...
static uint32_t sensorRegisterEx(const struct SensorInfo *si, TaggedPtr callInfo, void *callData, bool initComplete)
{
    ...
    //osLog(LOG_INFO, "%s-%u, %lu, %ld\n", __func__, __LINE__, *(mSensorsUsed->words), idx);
...
static void sensorInternalFwStateChanged(void *evtP)
{
    ...
    if (s) {
	    ...
	    //osLog(LOG_INFO, "%s-%u:%lu,%llu\n", __func__, __LINE__, s->currentRate, s->currentLatency);
...
bool sensorRequest(uint32_t unusedTid, uint32_t sensorHandle, uint32_t rate, uint64_t latency)
{
    ...
    //osLog(LOG_INFO, "%s-%u:%lu\n", __func__, __LINE__, newSensorRate);
    ...
    //osLog(LOG_INFO, "%s-%u:%llu, %llu\n", __func__, __LINE__, samplingPeriod, latency);
    ...
    //osLog(LOG_INFO, "%s-%u:onchange:%d\n", __func__, __LINE__, s->hasOnchange);
...
bool sensorRequestRateChange(uint32_t unusedTid, uint32_t sensorHandle, uint32_t newRate, uint64_t newLatency)
{
    ...
    //osLog(LOG_INFO, "%s-%u:%lu,%llu,%llu\n", __func__, __LINE__, newSensorRate, newLatency, samplingPeriod);
