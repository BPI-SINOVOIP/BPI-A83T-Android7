...
static uint32_t queryAppInfo(void *rx, uint8_t rx_len, void *tx, uint64_t timestamp)
{
	...
	//osLog(LOG_ERROR, "%s-%u,%llu, %lu, %lu\n", __func__, __LINE__, resp->appId, resp->appVer, resp->appSize);
        return sizeof(*resp);
    }
...
void nanohubPrefetchTx(uint32_t interrupt, uint32_t wakeup, uint32_t nonwakeup)
{
    ...
    do {
    	//osLog(LOG_ERROR, "%s-%u:%lu,%u\n", __func__, __LINE__, atomicReadByte(&mTxCurrLength), mTxNextLength);
	    ...
	    //osLog(LOG_ERROR, "%s-%u:mTxCurrLength:%lu\n", __func__, __LINE__, atomicReadByte(&mTxCurrLength));
            mTxNextLength = 0;
        }
