...
static void i2cCallback(void *cookie, size_t tx, size_t rx, int err)
{
	...
	// osLog(LOG_INFO, DRIVER_NAME "i2cCallback\n");
...
static bool sensorRateAls(uint32_t rate, uint64_t latency, void *cookie)
{
    ...
    if (mData.alsTimerHandle)
        timTimerCancel(mData.alsTimerHandle);
	mData.alsRate = sensorTimerLookupCommon(supportedRates, rateTimerVals, rate);
    mData.alsTimerHandle = timTimerSet(sensorTimerLookupCommon(supportedRates, rateTimerVals, rate), 0, 50, alsTimerCallback, NULL, true);
...
static bool sensorRateProx(uint32_t rate, uint64_t latency, void *cookie)
{
    ...
    if (mData.proxTimerHandle)
        timTimerCancel(mData.proxTimerHandle);
	mData.proxRate = sensorTimerLookupCommon(supportedRates, rateTimerVals, rate);
	mData.proxTimerHandle = timTimerSet(sensorTimerLookupCommon(supportedRates, rateTimerVals, rate), 0, 50, proxTimerCallback, NULL, true);
...
static void handle_i2c_event(struct I2cTransfer *xfer)
{
    ...
    case SENSOR_STATE_VERIFY_ID:
		osLog(LOG_ERROR, DRIVER_NAME "ams id 0x%x\n",xfer->txrxBuf.bytes[0]);
        /* Check the sensor ID */
        if (xfer->err != 0 || ((xfer->txrxBuf.bytes[0] != AMS_TMD2772_ID)&&(xfer->txrxBuf.bytes[0] != AMS_TMD2771_ID))) {
...
#if I2C_WR_DEBUG
		nextXfer = allocXfer(SENSOR_STATE_WR_TEST);
		if (nextXfer != NULL) {
			nextXfer->txrxBuf.bytes[0] = AMS_TMD2772_REG_ATIME;
			nextXfer->txrxBuf.bytes[1] = 0x5A;
			nextXfer->txrxBuf.bytes[2] = 0xA5;
			nextXfer->txrxBuf.bytes[3] = 0x55;
			i2cMasterTx(I2C_BUS_ID, I2C_ADDR, nextXfer->txrxBuf.bytes, 4,i2cCallback, nextXfer);
			}
		break;
	...
	case SENSOR_STATE_WR_TEST:
		if( read_cnt < 10)
			osLog(LOG_INFO, DRIVER_NAME "%x,%x,%x,%x\n",
				xfer->txrxBuf.bytes[0],xfer->txrxBuf.bytes[1],xfer->txrxBuf.bytes[2],xfer->txrxBuf.bytes[3]);
		else break;
		read_cnt++;
		nextXfer = allocXfer(SENSOR_STATE_WR_TEST);
		if (nextXfer != NULL) {
            nextXfer->txrxBuf.bytes[0] = AMS_TMD2772_REG_ATIME;
			nextXfer->txrxBuf.bytes[1] = 0;
			nextXfer->txrxBuf.bytes[2] = 0;
			nextXfer->txrxBuf.bytes[3] = 0;
            i2cMasterTxRx(I2C_BUS_ID, I2C_ADDR, nextXfer->txrxBuf.bytes, 1, nextXfer->txrxBuf.bytes, 3, i2cCallback, nextXfer);
        }
		break;
...
static bool init_app(uint32_t myTid)
{
    ...
    mData.lastAlsSample.idata = AMS_TMD2772_ALS_INVALID;
    mData.proxState = PROX_STATE_INIT;
	mData.alsRate = rateTimerVals[0];
	mData.proxRate = rateTimerVals[0];
...
static void handle_event(uint32_t evtType, const void* evtData)
{
    ...
    case EVT_SENSOR_PROX_TIMER:
        ...
        if (!mData.alsReading && !mData.proxReading) {
			//osLog(LOG_ERROR, DRIVER_NAME "read data timer\n");
		...
		if (mData.alsTimerHandle)
			timTimerCancel(mData.alsTimerHandle);
		mData.alsTimerHandle = timTimerSet(mData.alsRate, 0, 50, alsTimerCallback, NULL, true);
		mData.proxTimerHandle = timTimerSet(mData.proxRate, 0, 50, proxTimerCallback, NULL, true);
        break;
    }
}
