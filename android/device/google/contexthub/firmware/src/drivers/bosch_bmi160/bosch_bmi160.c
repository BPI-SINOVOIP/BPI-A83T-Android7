...
#define BMI160_SPI_BUS_ID         0
#define BMI160_SPI_SPEED_HZ       8000000
#define BMI160_SPI_MODE           3

#define BMI160_INT_IRQ            R_GPIOL_IRQn
#define BMI160_INT1_PIN           GPIO_PL(18)
#define BMI160_INT2_PIN           GPIO_PL(19)
...
#define SPI_WRITE_0(addr, data) spiQueueWrite(addr, data, 10)
...
#define SPI_READ_0(addr, size, buf) spiQueueRead(addr, size, buf, 10)
...
static void spiQueueRead(uint8_t addr, size_t size, uint8_t **buf, uint32_t delay)
{
    ...
    *buf = &T(txrxBuffer[T(mWbufCnt)]);
    memset((void *)(*buf), 0, size); // please double check whether need or not
        ...
        SpiCbkF callback, void *cookie, const char * src)
{
    ...
    T(mRegCnt) = 0;
    T(mWbufCnt) = 0;
	//ERROR_PRINT("aw==== spiBatchTxRx regCount : %d\n",regCount);
...
static bool enableInterrupt(struct Gpio *pin, struct ChainedIsr *isr)
{
	...
	gpioConfigAlt(pin, GPIO_SPEED_LOW, GPIO_PULL_DOWN, 0, GPIO_AF_EINT);
    extiEnableIntGpio(pin, EXTI_TRIGGER_RISING);
...
static bool accSetRate(uint32_t rate, uint64_t latency, void *cookie)
{
    ...
    if (trySwitchState(SENSOR_CONFIG_CHANGING)) {
		...
		osLog(LOG_INFO,"acc odr %d, osr %d\n",odr,osr);
...
static bool gyrSetRate(uint32_t rate, uint64_t latency, void *cookie)
{
    ...
    if (trySwitchState(SENSOR_CONFIG_CHANGING)) {
        ...
        // filtered samples
        SPI_WRITE(BMI160_REG_FIFO_DOWNS, (mTask.acc_downsample << 4) | mTask.gyr_downsample | 0x88);
		osLog(LOG_INFO,"gyro odr %d, osr %d\n",odr,osr);
        // flush the data and configure the fifo
        configFifo();
...
static void parseRawData(struct BMI160Sensor *mSensor, uint8_t *buf, float kScale, uint64_t sensorTime)
{
	...
	//osLog(LOG_ERROR, "bmi160:id %d, x: %d, y: %d, z: %d\n", mSensor->idx,(int)(1000*x), (int)(1000*y), (int)(1000*z));
...
static void timeSyncEvt(uint32_t evtGeneration, bool evtDataValid)
{
	...
	//FIXME:Mask temperature reg operation
	//Because read temperature reg will lead to spi loop timeout.
	//Maybe the Sensor temperature is too high.
...
static void processPendingEvt(void)
{
	...
	#if 0
	//fixme int 1 always acquire pending thing
    for (i = FIRST_CONT_SENSOR; i < NUM_OF_SENSOR; i++) {
        if (mTask.pending_config[i]) {
            mTask.pending_config[i] = false;
            configEvent(&mTask.sensors[i], &mTask.sensors[i].pConfig);
...
static void sensorInit(void)
{
    ...
    case INIT_BMI160:
        ...
        //FIXME:must delay some time before second step
        SPI_READ(BMI160_REG_INT_STATUS_0, 4, &mTask.statusBuffer, 10000);
...
static void handleSpiDoneEvt(const void* evtData)
{
    ...
    case SENSOR_VERIFY_ID:
		ERROR_PRINT("aw==== bmi160 match: %02x\n", mTask.dataBuffer[1]);
...
#if TEST_BMI160
		SET_STATE(SENSOR_TEST);
		SPI_WRITE(BMI160_REG_OFFSET_0, 0x55,100);
		SPI_WRITE(BMI160_REG_OFFSET_1, 0xAA,100);
		SPI_WRITE(BMI160_REG_OFFSET_2, 0xA5,100);
		SPI_READ(BMI160_REG_OFFSET_0, 3, &mTask.dataBuffer);
		spiBatchTxRx(&mTask.mode, sensorSpiCallback, &mTask, "superm test");
#else
...
#if TEST_BMI160
    case SENSOR_TEST:
	ERROR_PRINT("aw====:BMI160_REG_OFFSET_0 %02lx, %02x,%02x,%2x\n", (uint32_t)mTask.dataBuffer, mTask.dataBuffer[1],mTask.dataBuffer[2],mTask.dataBuffer[3]);
	SET_STATE(SENSOR_BOOT);
	SPI_WRITE(BMI160_REG_OFFSET_0, 0x55,100);
	SPI_WRITE(BMI160_REG_OFFSET_1, 0xAA,100);
	SPI_WRITE(BMI160_REG_OFFSET_2, 0xA5,100);
    ...
    case SENSOR_INITIALIZING:
        if (mTask.init_state == INIT_DONE) {
            ERROR_PRINT("Done initialzing, system IDLE\n");
	...
	//FIXME:Because we mask temperature reg operation. So we set temperature to be suitable value
	temperature16 = 0x4000; //temperature is 55 Celsius
...
static void handleEvent(uint32_t evtType, const void* evtData)
{
    ...
    case EVT_SENSOR_INTERRUPT_2:
        ...
        break;
    case EVT_TIME_SYNC:
	//ERROR_PRINT("aw==== EVT_TIME_SYNC,evtData : %ld\n",(uint32_t)evtData);
...
static bool startTask(uint32_t task_id)
{
    ...
    interrupt_enable(BMI160_INT_IRQ);
#if 0
    ERROR_PRINT("gpio config addr : 0x%x ,value : 0x%x\n",R_PIO_REG_BASE+0x08,readl(R_PIO_REG_BASE+0x08));
    ERROR_PRINT("gpio pull  addr : 0x%x ,value : 0x%x\n",R_PIO_REG_BASE+0x20,readl(R_PIO_REG_BASE+0x20));
    ERROR_PRINT("gpio external addr : 0x%x ,value : 0x%x\n",R_PIO_REG_BASE+0x208,readl(R_PIO_REG_BASE+0x208));
    ERROR_PRINT("gpio control  addr : 0x%x ,value : 0x%x\n",R_PIO_REG_BASE+0x210,readl(R_PIO_REG_BASE+0x210));
#endif
    //ERROR_PRINT("%s-%u\n", __func__, __LINE__);
