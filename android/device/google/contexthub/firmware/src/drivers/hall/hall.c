...
{
    //osLog(LOG_INFO, "%s-%u:%lu,%llu\n", __func__, __LINE__, rate, latency);
...
{
    union EmbeddedDataPoint sample;
    //osLog(LOG_INFO, "%llu:hall: delay: %lus\n", sensorGetTime(), (*(uint32_t *)data)++);
    (*(uint32_t *)data)++;
    //osLog(LOG_INFO, "hall: delay: %lus\n", (*(uint32_t *)data));

    if (mTask.on) {
    	sample.idata = (*(uint32_t *)data);
        if (sample.idata != mTask.prevReportedValue) {
            mTask.prevReportedValue = sample.idata;
	    //osLog(LOG_INFO, "hall: send data: %lu\n", sample.idata);
            osEnqueueEvt(sensorGetMyEventType(SENS_TYPE_HALL), sample.vptr, NULL);
        }
    }
}
...
static void handleEvent(uint32_t evtType, const void* evtData)
{
	...
	if (evtType == EVT_APP_START) {
		timTimerSet(100000, 0, 0, TimerCallback, &cnt, false);
		//timerId = timTimerSet(1000000, 0, 0, TimerCallback, &cnt, false);
		//osLog(LOG_INFO, "hall: started with tid %lu timerid %lu\n", mTask.id, timerId);
	    } else {
		//osLog(LOG_INFO, "hall: error evtType: %lu\n", evtType);
	    }
}
...
static bool startTask(uint32_t taskId)
{
    ...
    //osLog(LOG_INFO, "hall init finish\n");
