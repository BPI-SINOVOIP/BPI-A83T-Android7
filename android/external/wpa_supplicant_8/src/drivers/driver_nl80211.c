...
static int get_customized_macaddr(const char *ifname, u8 *mac)
{
	u8 * head, * end;
	int i;
	char val[PROPERTY_VALUE_MAX] = {0};

	if (property_get("wifi.use_customized_macaddr", val, NULL) <= 0 || strcmp(val, "0") == 0)
	{
		wpa_printf(MSG_DEBUG,"%s: Don't use_customized_macaddr\n ", __FUNCTION__);
		return -1;
	}

	if(property_get("ro.boot.mac", val, NULL))
	{
		int ret = sscanf(val, "%02hhx:%02hhx:%02hhx:%02hhx:%02hhx:%02hhx",
					&mac[0], &mac[1], &mac[2], &mac[3], &mac[4], &mac[5]) == 6;

		if(!ret)
		{
			wpa_printf(MSG_DEBUG,"%s: mac %s format error!\n ", __FUNCTION__, val);
			return -1;
		}

	} else {
		wpa_printf(MSG_DEBUG,"%s: ro.boot.mac is null\n ", __FUNCTION__);
		return -1;
	}

	if(strcmp("wlan0", ifname) == 0)
		/* net port bit set to 0 */
		mac[1] = (mac[1] & 0x0F) | 0x00;
	else if(strcmp("p2p0", ifname) == 0)
		/* net port bit set to 1 */
		mac[1] = (mac[1] & 0x0F) | 0x10;

	wpa_printf(MSG_DEBUG,
		"%s: Get %s mac %02x:%02x:%02x:%02x:%02x:%02x\n ", __FUNCTION__, ifname,
				      ...
				      void *global_priv)
{
	...
	if(get_customized_macaddr(ifname, mac) == 0)
		return wpa_driver_nl80211_drv_init(ctx, ifname, global_priv, 0, mac,
					   NULL);
