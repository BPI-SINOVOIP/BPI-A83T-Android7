...
#ifdef AW_MULTIPLE_THREAD_SUPPORT
class SkTaskBlend32_32: public SkTask_Base{
public:
        SkTaskBlend32_32(uint32_t _color,
                           uint32_t* SK_RESTRICT device,
                           size_t deviceRB,
                           int w,
                           int h,
                           int _x,
        ...
        virtual void go() {
            while(--height >= 0){
                SkBlitRow::Color32(dst, dst, width, color);
                dst = (uint32_t*)((char*)dst + rowbytes);
            }
        }
private:
        uint32_t    color;
        uint32_t* SK_RESTRICT dst;
        size_t rowbytes;
        int width;
        int height;
        int x;
...
void SkARGB32_Blitter::blitRect(int x, int y, int width, int height) {
    ...
    if(pThreadHandler->canUseThread()/* && (width*height) >= THREADHOLD*/){//TODO
        ...
        SkTaskBlend32_32* pTask =
            new SkTaskBlend32_32(color,device,rowBytes,width,(height >> 1), x, y);
        pThreadHandler->addTask(pTask);
        //main thread do other work.
        device = fDevice.writable_addr32(x,y+(height >> 1));
...
#ifdef AW_MULTIPLE_THREAD_SUPPORT
class SkTask_S32_D32_Shader : public SkTask_Base{
public:
    SkTask_S32_D32_Shader(SkShader::Context* _shader,
                                 int _x, int _y,
                                 int w, int h,
                                 uint32_t *device,
                                 size_t _rowBytes,
                                 SkShader::Context::ShadeProc _shadeProc,
                                 void* _ctx){
        shader = _shader;
        x = _x;
        y = _y;
        ...
        dst = device;
        rowBytes = _rowBytes;
        shadeProc = _shadeProc;
        ctx=_ctx;
    }
    ~SkTask_S32_D32_Shader(){}
    virtual void go() {
        do {
            shadeProc(ctx, x, y, dst, width);
            y += 1;
            dst = (uint32_t*)((char*)dst + rowBytes);
        } while (--height);
    }
private:
       ...
       uint32_t *dst;
       size_t rowBytes;
       SkShader::Context::ShadeProc shadeProc;
       void* ctx;
};
#endif /*AW_MULTIPLE_THREAD_SUPPORT*/
...
//add  new
        ...
        if(pThreadHandler->canUseThread()) {//TODO: Add a threadhold control
            //creat a sub task which will be handled by compute thread.
            SkTask_S32_D32_Shader* pTask =
                new SkTask_S32_D32_Shader(shaderContext,
                                         x,y,
                                         width,height >> 1,
                                         device,deviceRB,shadeProc,ctx);
            pThreadHandler->addTask(pTask);

            //main thread do other work
            device = (uint32_t*)((char*)device + deviceRB*(height >> 1));
            y += (height>>1);
            height -= (height>>1);
            do {
