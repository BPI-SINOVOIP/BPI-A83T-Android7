...
static void _gr_text(const GRFont* font, int x, int y, const char *s, bool bold);
void gr_text(const GRFont* font, int x, int y, const char *s, bool bold)
{
    if (SPLIT_SCREEN)
        _gr_text(font, x + gr_fb_width(), y, s, bold);
    return _gr_text(font, x, y, s, bold);
}
static void _gr_text(const GRFont* font, int x, int y, const char *s, bool bold)
{
...
static void _gr_texticon(int x, int y, GRSurface* icon);
void gr_texticon(int x, int y, GRSurface* icon) {
    if (SPLIT_SCREEN)
        _gr_texticon(x + gr_fb_width(), y, icon);
    _gr_texticon(x, y, icon);
}
static void _gr_texticon(int x, int y, GRSurface* icon) {
    if (icon == NULL) return;
...
static void _gr_fill(int x1, int y1, int x2, int y2);
void gr_fill(int x1, int y1, int x2, int y2)
{
    if (SPLIT_SCREEN) {
        int hw = gr_fb_width();
        _gr_fill(x1 + hw, y1, x2 + hw, y2);
    }
    _gr_fill(x1, y1, x2, y2);
}
static void _gr_fill(int x1, int y1, int x2, int y2)
{
...
static void _gr_blit(GRSurface* source, int sx, int sy, int w, int h, int dx, int dy);
void gr_blit(GRSurface* source, int sx, int sy, int w, int h, int dx, int dy) {
    if (SPLIT_SCREEN)
        _gr_blit(source, sx, sy, w, h, dx + gr_fb_width(), dy);
    _gr_blit(source, sx, sy, w, h, dx, dy);
}
static void _gr_blit(GRSurface* source, int sx, int sy, int w, int h, int dx, int dy) {
...
{
    if (SPLIT_SCREEN)
        return (gr_draw->width - 2*overscan_offset_x) / 2;
    else
