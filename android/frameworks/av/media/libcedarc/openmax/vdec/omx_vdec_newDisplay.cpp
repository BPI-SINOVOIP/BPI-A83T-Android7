...
#ifdef OMX_DROP_B_FRAME_4K
    nTimeUs1 = OMX_GetNowUs();
    //* refine BaseSysTime for CurtTme start from 0
    if (pSelf->bFirstSecond == OMX_TRUE && pSelf->mBaseTimeGap == 0)
    {
        pSelf->mBaseTimeGap = nTimeUs1 - pSelf->mBaseSysTime;
    }
    if (pSelf->bFirstSecond == OMX_TRUE)
    {
        nCurTimeUs = nTimeUs1 - pSelf->mBaseSysTime + pSelf->mBasePts - pSelf->mBaseTimeGap;
    }
    else
    {
        nCurTimeUs = nTimeUs1 - pSelf->mBaseSysTime + pSelf->mBasePts;
    }
    logv("nCurTimeUs = %lld, nSysCurTime = %lld, lastTime = %lld",nCurTimeUs, nTimeUs1, pSelf->mBaseSysTime);
...
#endif
    if(nTimeUs1 - pSelf->mBaseSysTime > DROP_REBASE_TIME_GAP)
    {
       pSelf->mFirstPictureFlag = OMX_TRUE;
       pSelf->bFirstSecond = OMX_FALSE;
    }
#else
...
int OmxCopyInputDataToDecoder(omx_vdec* pSelf)
{
    ...
    if(pInBufHdr->nTimeStamp >= 0)
    {
        ...
        DataInfo.bValid = 1;
        if(pSelf->mFirstPictureFlag)
        {
            pSelf->mBasePts = pInBufHdr->nTimeStamp;
            logv("**** mBasePts = %lld", pSelf->mBasePts);
            pSelf->mBaseSysTime = OMX_GetNowUs();
            if (pSelf->mBasePts == 0)
            {
                pSelf->bFirstSecond = OMX_TRUE;
            }
        }
    }
    else
