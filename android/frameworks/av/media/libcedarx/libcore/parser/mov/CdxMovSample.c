...
static CDX_S32 movTimeToSample(struct CdxMovParser *p, int seconds)
{
    MOVContext      *c;
...
/************************************************************************/
static CDX_S32 movFindKeyframe(struct CdxMovParser *p, CDX_S32 direction,
                        ...
                        CDX_S32 seconds)
{
    ...
    else
    {
        ...
        else
        {
            ...
            if(st->stts_size == 1)
            {
                chunk_num = calc_sample_parameter.seek_audio_sample - calc_sample_parameter.stsz_acc;
                if(chunk_num > 0)
                {
                    ...
                    //st->mov_idx_curr.stco_idx++;
                    st->mov_idx_curr.sample_idx = calc_sample_parameter.seek_audio_sample;
                    st->mov_idx_curr.current_dts = (CDX_U64)calc_sample_parameter.seek_audio_sample *
                    calc_sample_parameter.stts_sample_duration;
                }
            }
        }
...
static CDX_S16 movSeekKeySample(struct CdxMovParser *p, CDX_U32 sample_num, CDX_S32 seconds)
{
    ...
    CDX_S32         ret;
    CDX_S32         i;
    CDX_U64         video_time = 0,audio_time;
...
CDX_S32 MovSeekSample(struct CdxMovParser *p,MOVContext *c,CDX_S32 seekTime)
{
    CDX_S32        result;
    CDX_S32        frmidx = 0;
