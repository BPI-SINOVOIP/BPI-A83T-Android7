...
void android_os_Process_setThreadCpusetGroup(JNIEnv* env, jobject clazz, int tid, jint grp)
{
    ALOGV("%s tid=%d grp=%" PRId32, __func__, tid, grp);
    SchedPolicy sp = (SchedPolicy) grp;
    int res = set_cpuset_policy(tid, sp);
    if (res != NO_ERROR) {
        signalExceptionForGroupError(env, -res, tid);
    }
}
...
static void get_cpuset_cores_for_policy(SchedPolicy policy, cpu_set_t *cpu_set)
{
    ...
    switch (policy) {
        ...
        case SP_TOP_APP:
            ...
            break;
        case SP_VR_DAEMON:
            filename = "/dev/cpuset/vr-daemon/cpus";
            break;
        default:
            filename = NULL;
    }
...
static const JNINativeMethod methods[] = {
    ...
    {"setThreadCpusetGroup", "(II)V", (void*)android_os_Process_setThreadCpusetGroup},
