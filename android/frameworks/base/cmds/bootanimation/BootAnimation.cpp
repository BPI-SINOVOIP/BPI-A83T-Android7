...
static const int MAX_DECODE_CACHE = 2;
...
SkBitmap* BootAnimation::decode(FileMap* map)
{
...
status_t BootAnimation::initTexture(FileMap* map, int* width, int* height)
{
    ...
    return initTexture(decode(map), width, height);
}

status_t BootAnimation::initTexture(SkBitmap *bitmap, int* width, int* height)
{
...
status_t BootAnimation::readyToRun() {
    ...
    // by softwinner humingming@allwinnertech.com
    // the default screen orientation may landspace
    int finalW = dinfo.w;
    int finalH = dinfo.h;
    if (dinfo.orientation == DISPLAY_ORIENTATION_90
            || dinfo.orientation == DISPLAY_ORIENTATION_270) {
        finalW = dinfo.h;
        finalH = dinfo.w;
    }
    ALOGD("dinfo:(%dx%d), orient:%d, final:(%dx%d)",
            dinfo.w, dinfo.h, dinfo.orientation, finalW, finalH);
    sp<SurfaceControl> control = session()->createSurface(String8("BootAnimation"),
    //        dinfo.w, dinfo.h, PIXEL_FORMAT_RGB_565);
            finalW, finalH, PIXEL_FORMAT_RGB_565);
    ...
    ALOGD("egl surface size:(%dx%d)", mWidth, mHeight);
...
#ifdef SPLIT_SCREEN
    const int animationX = (mWidth / 2 - animation.width) / 2;
        ...
        for (int r=0 ; !part.count || r<part.count ; r++) {
            ...
            for (size_t j=0 ; j<fcount && (!exitPending() || part.playUntilComplete) ; j++) {
                ...
                if (r > 0) {
                    ...
                    int w, h;
#ifdef MULTITHREAD_DECODE
                    initTexture(frameManager->next(), &w, &h);
#else
                    initTexture(frame, &w, &h);
#endif
                }
...
#ifdef SPLIT_SCREEN
                clearReg.subtractSelf(Rect(mWidth / 2 + xc, yc, mWidth / 2 + xc+frame.trimWidth, yc+frame.trimHeight));
#endif
                if (!clearReg.isEmpty()) {
...
#ifdef SPLIT_SCREEN
                glDrawTexiOES(mWidth / 2 + xc, mHeight - (yc + frame.trimHeight),
                              0, frame.trimWidth, frame.trimHeight);
#endif
...
bool BootAnimation::playSoundsAllowed() const {
    ...
    bool FrameManager::DecodeThread::threadLoop()
    {
        ...
        if (work.frame != NULL) {
            work.bitmap = BootAnimation::decode(&(*work.frame->map));
