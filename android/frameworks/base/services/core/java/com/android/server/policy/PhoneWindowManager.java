...
public class PhoneWindowManager implements WindowManagerPolicy {
    ...
    private boolean disableDirectbootBlock;
    ...
    private boolean mIsVrBuild = false;
    ...
    private boolean getVrModeEnabled() {
    ...
    public void init(Context context, IWindowManager windowManager,
        ...
        disableDirectbootBlock=context.getResources().getBoolean(com.android.internal.R.bool.config_disableDirectBootBlock);
        ...
        mIsVrBuild = (SystemProperties.getInt("ro.sys.vr.build", 0) == 1);
    }
    ...
    public long interceptKeyBeforeDispatching(WindowState win, KeyEvent event, int policyFlags) {
        ...
        if (keyCode == KeyEvent.KEYCODE_HOME) {
            ...
            // for popup vr selector mode menu
    ...
    private void launchAssistAction(String hint, int deviceId) {
        sendCloseSystemWindows(SYSTEM_DIALOG_REASON_ASSIST);
        if ((!isUserSetupComplete()) || (SystemProperties.getInt("ro.sys.vr.build", 0) == 1)) {
            // Disable opening assist window during setup or vr device
            return;
        }
        Bundle args = null;
    ...
    public int interceptKeyBeforeQueueing(KeyEvent event, int policyFlags) {
        ...
        switch (keyCode) {
            ...
            // add by softwinner humingming@allwinnertech.com
            // for send boradcast pesesnor event to app(near and far send as a special key event)
            case 227:
            case 228: {
                if (mIsVrBuild) {
                    String action;
                    if (227 == keyCode) {
                        Log.i(TAG, "psensor is near, sendboradcast");
                        action = "com.softwinner.vr.action.psensor_near";
                    } else {
                        Log.i(TAG, "psensor is far, sendboradcast");
                        action = "com.softwinner.vr.action.psensor_far";
                    }
    ...
    private void wakeUpFromPowerKey(long eventTime) {
        ...
        // add by softwinner humingming@allwinnertech.com
        // our psensor service need know power on or ac plugin event
        // which there 2 will cost SCREEN_ON broadcast, we can't know from SCREEN_ON broadcast
        if (mIsVrBuild) {
            Log.i(TAG, "send power on broadcast");
            mContext.sendBroadcast(new Intent("com.softwinner.vr.action.power_on"));
        }
    }
    ...
    // add by softwinner humingming@allwinnertech.com
    // query from surfaceflinger whether we need show vr mode selector
    private boolean needShowVrModeSelector() {
        int ret = displayCtrl(0, CMD_QUERY, 0, 0);
        Slog.d(TAG, "displayCtrl: ret: " + ret);
        // query error let it show as default ...
        return (0 == ret) ? true: false;
    }

    private void showVrModeSelector() {
        if (mIsVrBuild && needShowVrModeSelector()) {
            PackageManager packageManager = mContext.getPackageManager();
            Intent intent = packageManager.getLaunchIntentForPackage("com.softwinner.VrModeSelector");
            if (intent != null) {
    ...
    private static final int CMD_QUERY = 9;

    private int displayCtrl(int disp, int cmd, int val0, int val1) {
        try {
            IBinder surfaceFlinger = ServiceManager.getService("SurfaceFlinger");
            if (null == surfaceFlinger) {
                Slog.e(TAG, "get surfaceFlinger null !");
                return 0;
            }
            ...
            Slog.e(TAG, "displayCtrl error: ", e);
            return 0;
        }
    }
