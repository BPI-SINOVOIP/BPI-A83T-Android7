...
final class ActivityRecord {
    ...
    private void reportLaunchTimeLocked(final long curTime) {
        final Resources resources = mStackSupervisor.mService.mContext.getResources();
        boolean boot_opt = resources.getBoolean(R.bool.config_targetUseBootupOpz);
        String launcher_shortComponentName = resources.getString(R.string.launcher_shortComponentName);
        if (boot_opt) {
            if (shortComponentName.equals(launcher_shortComponentName)) {
    ...
    void windowsVisibleLocked() {
        ...
        if (service.testIsSystemReady()) {
            PowerManager pm= (PowerManager)service.getPowerManagerInstance();
            if (mStackSupervisor.mIsExtremeMode != true && mStackSupervisor.mIsPerfLockAcquired == true) {
                //boost down perf
            pm.powerHint(PowerManager.NORMAL,1);
            mStackSupervisor.mIsPerfLockAcquired = false;
            } else if ("com.android.launcher3".equals(this.processName)) {
