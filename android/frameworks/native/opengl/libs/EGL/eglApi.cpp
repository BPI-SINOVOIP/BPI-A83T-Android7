...
#include "allwinner/AwPortalDevice.h"
...
static const extention_map_t sExtensionMap[] = {
    ...
    // allwinnertech platform specific apis, used for vr project.
    { "eglCustomEnablePortal", (__eglMustCastToProperFunctionPointerType)&eglCustomEnablePortal},
    { "eglCustomPrepareFrame", (__eglMustCastToProperFunctionPointerType)&eglCustomPrepareFrame},
    { "eglCustomStartHeadTracker", (__eglMustCastToProperFunctionPointerType)&eglCustomStartHeadTracker},
    { "eglCustomStopHeadTracker", (__eglMustCastToProperFunctionPointerType)&eglCustomStopHeadTracker},
};
                                    ...
                                    const EGLint *attrib_list)
{
    ...
    ALOGD("call %s. window=%p",__func__, window);

    bool useHwSurface = false;
    if(GetPortalDevice() != NULL && GetPortalDevice()->IsPortalEnabled() == true)
    {
        ANativeWindow *hwWindow = NULL;
        if(0 == GetPortalDevice()->CreatePortal(&hwWindow))
        {
            ALOGD("do replace hw surface. tid=%d", gettid());
            useHwSurface = true;
            window = (NativeWindowType)hwWindow;
        }
    }
    ...
    if (dp) {
        ...
        if (surface != EGL_NO_SURFACE) {
            ...
            // start portal success
            if(useHwSurface == true)
            {
                ALOGD("save hw surface");
                GetPortalDevice()->SavePortalSurface(s);
            }
            return s;
        }
...
EGLBoolean eglDestroySurface(EGLDisplay dpy, EGLSurface surface)
{
    ...
    bool matched = GetPortalDevice() != NULL && GetPortalDevice()->GetPortalSurface() == surface;
    ...
    if(matched)
    {
        ALOGD("PortalApi: diable portal.");
        GetPortalDevice()->DestroyPortal();
    }
    return result;
}
...
{
    EGLBoolean result = eglSwapBuffersWithDamageKHR(dpy, surface, NULL, 0);

    if(NULL != GetPortalDevice() && surface == GetPortalDevice()->GetPortalSurface())
    {
        GetPortalDevice()->Reprojection();
    }
    return result;
}
