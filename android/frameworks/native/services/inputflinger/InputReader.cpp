...
void KeyboardInputMapper::processKey(nsecs_t when, bool down, int32_t scanCode,
    ...
    ALOGD("receive keycode=%d", keyCode);

    if (propertyGetBool("sys.boot_completed", false) &&
        (propertyGetBool(VRMODE, false) && propertyGetBool(VRDESKTOP, false) &&
            propertyGetBool(GAZECURSOR_INTERACTIVE, true))) {
        bool consume = FakeInputDispatcher::getInstance()->notifyKey(&args, getListener());
        ALOGD("comsume keycode=%d, ret=%d", keyCode, consume);
        if(consume){
            return;
        }
    }
...
void CursorInputMapper::sync(nsecs_t when) {
    ...
    if (downChanged || moved || scrolled || buttonsChanged) {
        ...
        NotifyMotionArgs args(when, getDeviceId(), mSource, policyFlags,
                ...
                displayId, 1, &pointerProperties, &pointerCoords,
                mXPrecision, mYPrecision, downTime);
        notifyListenerMotion(getListener(), &args);
...
bool propertyGetBool(const char* key, bool defaultValue)
{
    int len;
...
void notifyListenerMotion(InputListenerInterface* listener, NotifyMotionArgs* args) {
    // TODO: delete true
    if (propertyGetBool("sys.boot_completed", false) &&
        (propertyGetBool(VRMODE, false) && propertyGetBool(VRDESKTOP, false) && propertyGetBool(GAZECURSOR_INTERACTIVE, true))) {
        FakeInputDispatcher::getInstance()->notifyMotion(args);
    }
