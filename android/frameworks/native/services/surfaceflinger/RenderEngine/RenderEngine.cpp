...
#include "VrDesktop/VrDesktopRenderEngine.h"
...
#warning "using EGL_IMG_context_priority"
    ...
    case GLES_VERSION_3_0:
        ...
        engine = new VrDesktopRenderEngine();
        break;
    }
...
static status_t selectConfigForAttribute(EGLDisplay dpy, EGLint const* attrs,
    ...
    if (n) {
        if (attribute != EGL_NONE) {
            ...
            //in case not find, pick the first one
            *outConfig = configs[0];
            delete [] configs;
            return NO_ERROR;
        } else {
...
static status_t selectEGLConfig(EGLDisplay display, EGLint format,
    ...
    if (renderableType) {
        ...
        // try MSAA
        wantedAttribute                         = EGL_SAMPLES;
        wantedAttributeValue                    = 4;
    } else {
