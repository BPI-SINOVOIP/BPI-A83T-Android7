...
void SurfaceFlinger::init() {
    ...
    { // Autolock scope
        ...
        //init the vr app list
        mVrAppList = new VrDesktopAppList();
    }
...
void SurfaceFlinger::sendControllerData(int status, int needRecenter,
                                        float x, float y, float z, float w, float l)
{
    getRenderEngine().sendControllerData(status, needRecenter, x, y, z, w, l);
}

int SurfaceFlinger::sendDispalyData(int disp, int cmd, int val0, int val1)
{
    ALOGV("disp=%d, val1=%d", disp, val1);
    if (cmd == 7) {
        // get vr mode.
        ALOGV("layer name = %s: ", lastLayer.string());
        bool isvr = mVrAppList->isVrApp(lastLayer);
        if (isvr) {
            return 1;
        } else {
            return 0;
        ...
        if (!lastLayer.isEmpty()) {
            if (val0 == 1) {
                mVrAppList->addTempVrApp(lastLayer);
                ALOGV("Need VR Mode, add %s to Vr App List", lastLayer.string());
            } else {
                mVrAppList->deleteTempVrApp(lastLayer);
            }
        }
    } else if (cmd == 9) {
        bool isFix = mVrAppList->isFixedApp(lastLayer);
        if (isFix) {
            return 1;
        } else {
            return 0;
...
int getProperty(const char* key, char* value, const char* def){
    ATRACE_CALL();
    return property_get(key, value, def);
}
...
void SurfaceFlinger::vrModeSwitch(){
    ATRACE_CALL();
    if(!mBootFinished){
        return;
    }
    //check whether the vr mode is enabled.
    char value[2];
    //property_get("persist.vr.enable", value, "0");
    getProperty("persist.vr.enable", value, "0");
    if(atoi(value)){
        //if vr mode is enabled, check whether we should use the vrdesktop function;
        ...
        for (size_t d = 0; d < mDisplays.size(); d++) {
            ...
            for (auto& layer : currentLayers) {
                /* get last layer. */
                if (i == 0) {
                    if (layer->getName().find("com.softwinner.VrModeSelector") <= -1
                        && layer->getName().find("StatusBar") <= -1
                        && layer->getName().find("animation background stackId=0") <= -1
                        && layer->getName().find("animation background stackId=1") <= -1
                        && layer->getName().find("animation background stackId=2") <= -1
                        && layer->getName().find("ColorFade") <= -1) {
                        lastLayer = layer->getName().string();
                    }
                }
                i++;
            }
        ...
        for (size_t d = 0; d < mDisplays.size(); d++) {
            ...
            for (i = 0; i < count; i++) {
                ...
                // show vr app in normal mode.
                if(mVrAppList->isVrApp(layer->getName())){
                    if(mFirstVrDeskBoot || mVrDesktopEnable){
                        ALOGD("switch back to normal display mode");
                        property_set("service.vrdesktop.enable", "0");
                        getRenderEngine().enableVrDesktop(false);
                        mVrDesktopEnable = false;
                        mFirstVrDeskBoot = false;
                    }
                    return;
                }
            }
            //show the normal app in vrdesktop mode
            if(i == count && (mFirstVrDeskBoot || !mVrDesktopEnable)){
                ALOGD("switch to vr desktop mode");
                getRenderEngine().recenterOrientation();
                property_set("service.vrdesktop.enable", "1");
                getRenderEngine().enableVrDesktop(true);
                mVrDesktopEnable = true;
                mFirstVrDeskBoot = false;
                return;
            }
        }
        ...
        for (size_t dpy=0 ; dpy<mDisplays.size() ; dpy++) {
            ...
            if (hwcId >= 0) {
                ...
                for (auto& layer : currentLayers) {
                    ...
                    if (mDebugDisableHWC || mDebugRegion || mVrDesktopEnable) {
                        layer->forceClientComposition(hwcId);
                    }
                }
            }
        ...
        const sp<const DisplayDevice>& displayDevice, const Region& dirty)
{
    ...
    if (!needSwitch) {
        // when using virtual display, don't draw forground.
        if(mVrDesktopEnable) {
            mRenderEngine->drawForground();
        }
    }
...
void SurfaceFlinger::onInitializeDisplays() {
    ...
    // add by softwinner humingming@allwinnertech.com
    // default orientation for vr app
    //d.orientation = DisplayState::eOrientationDefault;
    char value[PROPERTY_VALUE_MAX];
    property_get("ro.sys.vr.forcelandscape", value, "0");
    int defaultRotation = atoi(value);
