...
public:
    ...
    virtual void sendControllerData(int status, int needRecenter,
            float x, float y, float z, float w, float l){
        Parcel data, reply;
        data.writeInterfaceToken(ISurfaceComposer::getInterfaceDescriptor());
        data.writeInt32(status);
        data.writeInt32(needRecenter);
        data.writeFloat(x);
        data.writeFloat(y);
        data.writeFloat(z);
        data.writeFloat(w);
        data.writeFloat(l);
        remote()->transact(BnSurfaceComposer::SEND_CONTROLLER_DATA, data, &reply);
    }

    virtual int sendDispalyData(int disp, int cmd, int val0, int val1) {
        Parcel data, reply;
        data.writeInterfaceToken(ISurfaceComposer::getInterfaceDescriptor());
        ...
        remote()->transact(BnSurfaceComposer::SEND_DISPLAY_DATA, data, &reply);
        return reply.readInt32();
    }
};
    ...
    uint32_t code, const Parcel& data, Parcel* reply, uint32_t flags)
{
    switch(code) {
        ...
        case SEND_CONTROLLER_DATA: {
            CHECK_INTERFACE(ISurfaceComposer, data, reply);
            int status, needRecenter;
            float x, y, z, w, l;
            data.readInt32(&status);
            data.readInt32(&needRecenter);
            data.readFloat(&x);
            data.readFloat(&y);
            data.readFloat(&z);
            data.readFloat(&w);
            data.readFloat(&l);
            sendControllerData(status, needRecenter, x, y, z, w, l);
            return NO_ERROR;
        }
        case SEND_DISPLAY_DATA: {
            CHECK_INTERFACE(ISurfaceComposer, data, reply);
            int disp, cmd, val0, val1, result;
            data.readInt32(&disp);
            data.readInt32(&cmd);
            data.readInt32(&val0);
            data.readInt32(&val1);
            result = sendDispalyData(disp, cmd, val0, val1);
            reply->writeInt32(result);
            return NO_ERROR;
        }
        default: {
