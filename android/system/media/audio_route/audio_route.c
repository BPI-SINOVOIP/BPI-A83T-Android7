    ...
    for (i = 0; i < ar->num_mixer_ctls; i++) {
        ...
        if (type == MIXER_CTL_TYPE_BYTE) {
            for (j = 0; j < num_values; j++) {
                mixer_value = mixer_ctl_get_value(ctl, j);
                if (ar->mixer_state[i].old_value.bytes[j] != ar->mixer_state[i].new_value.bytes[j]
                        || mixer_value != ar->mixer_state[i].new_value.integer[j]) {
                    changed = true;
                    break;
                }
            }
             ...
             for (j = 0; j < num_values; j++) {
                 mixer_value = mixer_ctl_get_value(ctl, j);
                 if (ar->mixer_state[i].old_value.enumerated[j] != ar->mixer_state[i].new_value.enumerated[j]
                         || mixer_value != ar->mixer_state[i].new_value.integer[j]) {
                     changed = true;
                     break;
                 }
...
unsigned int audio_route_get_value_by_name(struct audio_route *ar, const char *name)
{
    unsigned int val = 0;
    enum mixer_ctl_type type;
    unsigned int num_values;
...
void audio_route_set_value(struct audio_route *ar, int id, int value, unsigned int num_values)
{
    struct mixer_ctl *ctl;
    enum mixer_ctl_type type;
