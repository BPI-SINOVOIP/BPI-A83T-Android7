...
status_t PublicVolume::doMount() {
    ...
    if (!vfat::Check(mDevPath)) {
        LOG(INFO) << getId() << " check vfat filesystem OK";
    } else if (!ntfs::Check(mDevPath)) {
        LOG(INFO) << getId() << " check ntfs filesystem OK";
    } else {
        LOG(ERROR) << getId() << " failed " << mFsType << " filesystem check";
        return -EIO;
    }
    ...
    if (!vfat::Mount(mDevPath, mRawPath, false, false, false,
        ...
        LOG(INFO) << getId() << " mount vfat filesystem OK";
    } else if (!ntfs::Mount(mDevPath, mRawPath, false, false, false,
            AID_MEDIA_RW, AID_MEDIA_RW, 0007, false)) {
        LOG(INFO) << getId() << " mount ntfs filesystem OK";
    } else {
        PLOG(ERROR) << getId() << " failed to mount " << mFsType << " " << mDevPath;
        return -EIO;
    }
