...
#include "fec/io.h"
#include "ext4_sb.h"
#include "../extras/squashfs_utils/squashfs_utils.h"
#include "fs_mgr.h"
        ...
        char **table)
{
    ...
    *table = (char*)malloc(table_length + 1);
    if (!*table) {
...
static int compare_last_signature(struct fstab_rec *fstab, int *match)
{
    ...
    *match = 1;
	...
	char verity_block[] = VERITY_BLOCK;
...
int fs_mgr_setup_verity(struct fstab_rec *fstab)
{
    ...
    char *verity_table_signature = NULL;
	...
	// get verity_block partition size
	char verity_block[] = VERITY_BLOCK;
    if (get_fs_size(fstab->fs_type, verity_block, &device_size) < 0) {
        return retval;
    }

	// read the verity_table_signature and verity_table from verity_block
	...
	// read verity_table_signature and verity_table fail
	if (retval < 0) {
        goto out;
    }
    ...
    // verify the signature on the verity_table
