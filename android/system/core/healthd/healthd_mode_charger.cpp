...
#ifndef BOARD_POWER_ON_KEY_TIME
  #define POWER_ON_KEY_TIME       (2 * MSEC_PER_SEC)
#else
  #define POWER_ON_KEY_TIME      (BOARD_POWER_ON_KEY_TIME * MSEC_PER_SEC)
#endif
#ifndef BOARD_UNPLUGGED_SHUTDOWN_TIME
  #define UNPLUGGED_SHUTDOWN_TIME (10 * MSEC_PER_SEC)
#else
  #define UNPLUGGED_SHUTDOWN_TIME (BOARD_UNPLUGGED_SHUTDOWN_TIME * MSEC_PER_SEC)
#endif
...
static void draw_capacity(struct charger *charger)
{
    ...
    struct android::animation::frame *frame = &batt_anim->frames[batt_anim->cur_frame];

    snprintf(cap_str, (STR_LEN - 1), "%d%%", charger->batt_anim->cur_level);
    str_len_px = gr_measure(gr_sys_font(), cap_str);
    x = (gr_fb_width() - str_len_px) / 2;
    h = gr_get_height(frame->surface);
    y = (gr_fb_height() + h + char_height) / 2 ;
    android_green();
...
static void process_key(struct charger *charger, int code, int64_t now)
{
    ...
    LOGW("now:%" PRId64 "key->pending:%" PRId64 " run:%" PRId64 "\n", now,key->pending,charger->batt_anim->run);
    if (code == KEY_POWER) {
        if (key->down) {
