...
static int vr_cpuset_fd = -1; // special cpuset for vr
#endif
...
static void __initialize(void) {
    ...
    if (!access("/dev/cpuset/tasks", F_OK)) {
        ...
        filename = "/dev/cpuset/vr-daemon/tasks";
        vr_cpuset_fd = open(filename, O_WRONLY | O_CLOEXEC);
...
int get_sched_policy(int tid, SchedPolicy *policy)
{
    ...
    if (__sys_supports_schedgroups) {
        ...
        if (grpBuf[0] == '\0') {
            ...
            *policy = SP_TOP_APP;
        } else if (!strcmp(grpBuf, "vr-daemon")) {
            *policy = SP_VR_DAEMON;
        } else {
            errno = ERANGE;
            return -1;
