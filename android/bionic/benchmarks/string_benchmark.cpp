...
static void BM_string_strcpy(benchmark::State& state) {
    const size_t nbytes = state.range_x();
    char* src = new char[nbytes]; char* dst = new char[nbytes+10];
    memset(src, 'x', nbytes);
    while (state.KeepRunning()) {
        strcpy(dst, src);
    }
...
static void BM_string_strrchr(benchmark::State& state) {
    const size_t nbytes = state.range_x();
    char* src = new char[nbytes]; uint64_t dst=(uint64_t)nbytes%128;
    memset(src, 'x', nbytes); char *ret=NULL;
    while (state.KeepRunning()) {
        ret=strrchr(src,dst);
    }
...
static void BM_string_memchr(benchmark::State& state) {
    ...
    memset(buf, 'x', nbytes);
    char chr=state.range_x()%25+'a';
    while (state.KeepRunning()) {
        res = (char *)memchr(buf, chr, nbytes);
    }
    state.SetBytesProcessed(uint64_t(state.iterations()) * uint64_t(nbytes));
    delete[] buf;
}
